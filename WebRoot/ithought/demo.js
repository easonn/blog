...
window.Raphael = (function() {
	var ac = /[, ]+/, g = document, P = window, U = {
		...
		if (f.is(arguments[0], "array")) {
			var e = arguments[0], R = m[h](f, e.splice(0, 3 + f.is(e[0],
					"number"))), af = R.set();
		...
		x : "number",
		y : "number"
	}, y = [ "click", "dblclick", "mousedown", "mousemove", "mouseout",
			"mouseover", "mouseup" ], o = "prototype", h = "apply", C = "length", s = "parseFloat", r = "parseInt";
	f.version = "1.0";
	f.type = (window.SVGAngle
	...
	f.is = function(i, e) {
		...
		return Object[o].toString.call(i).replace(/^\[object\s+|\]$/gi, "")
				.toLowerCase() == e;
	};
	...
	f.hsb2rgb = ab(
			function(aj, ah, an) {
				...
				var ag = Math.floor(aj * 6), ak = (aj * 6) - ag, ad = an
						* (1 - ah), R = an * (1 - (ah * ak)), ap = an
						* (1 - (ah * (1 - ak)));
				...
				var al = {
					...
					g : af,
					b : ao
				}, e = (~~ae).toString(16), ai = (~~af).toString(16), am = (~~ao)
						.toString(16);
				if (e[C] == 1) {
					e = "0" + e;
				}
	...
	function ab(ad, i, e) {
		function R() {
			var ae = Array[o].splice.call(arguments, 0, arguments[C]), af = ae
					.join("\u25ba");
			R.cache = R.cache || {};
			R.count = R.count || [];
			if (af in R.cache) {
	...
	f.getRGB = ab(
			function(e) {
				...
				var R, ad, aj, ah = (e + "")
						.match(/^\s*((#[a-f\d]{6})|(#[a-f\d]{3})|rgb\(\s*([\d\.]+\s*,\s*[\d\.]+\s*,\s*[\d\.]+)\s*\)|rgb\(\s*([\d\.]+%\s*,\s*[\d\.]+%\s*,\s*[\d\.]+%)\s*\)|hs[bl]\(\s*([\d\.]+\s*,\s*[\d\.]+\s*,\s*[\d\.]+)\s*\)|hs[bl]\(\s*([\d\.]+%\s*,\s*[\d\.]+%\s*,\s*[\d\.]+%)\s*\))\s*$/i);
				if (ah) {
					if (ah[2]) {
					...
					if (ah[3]) {
						...
						ad = P[r](
								ah[3].substring(2, 3) + ah[3].substring(2, 3),
								16);
						R = P[r](ah[3].substring(1, 2) + ah[3].substring(1, 2),
								16);
					}
					if (ah[4]) {
						ah = ah[4].split(/\s*,\s*/);
					...
					var i = (~~R).toString(16), af = (~~ad).toString(16), ai = (~~aj)
							.toString(16), ak = /^(?=\d$)/, ae = "replace";
					i = i[ae](ak, "0");
					af = af[ae](ak, "0");
					ai = ai[ae](ak, "0");
					ah.hex = "#" + i + af + ai;
	...
	var H = ab(function(ak) {
		...
		for (var ae = 0, aj = ak[C]; ae < aj; ae++) {
			if (ak[ae][0] == "M") {
				...
				var af = N(ah, ag, ak[ae][1], ak[ae][2], ak[ae][3], ak[ae][4],
						ak[ae][5], ak[ae][6]);
		...
		var af = 1 / 3, ah = 2 / 3;
		return [ af * i + ah * ag, af * ad + ah * ae, af * e + ah * ag,
				af * R + ah * ae, e, R ];
	}, u = function(ap, aU, az, ax, aq, ak, af, ao, aT, ar) {
		var ae = Math.PI, aw = ae * 120 / 180, e = ae / 180 * (+aq || 0), aD = [], aA, aQ = ab(function(
				aV, aY, i) {
			var aX = aV * Math.cos(i) - aY * Math.sin(i), aW = aV * Math.sin(i)
					+ aY * Math.cos(i);
			return {
				x : aX,
				y : aW
		...
		if (!ar) {
			...
			var ad = az * az, aI = ax * ax, aK = (ak == af ? -1 : 1)
					* Math.sqrt(Math
							.abs((ad * aI - ad * aE * aE - aI * aF * aF)
									/ (ad * aE * aE + aI * aF * aF))), au = aK
					* az * aE / ax + (ap + ao) / 2, at = aK * -ax * aF / az
					+ (aU + aT) / 2, aj = Math.asin((aU - at) / ax), ai = Math
					.asin((aT - at) / ax);
			aj = ap < au ? ae - aj : aj;
			ai = ao < au ? ae - ai : ai;
			aj < 0 && (aj = ae * 2 + aj);
		...
		an = ai - aj;
		var ah = Math.cos(aj), aS = Math.sin(aj), ag = Math.cos(ai), aR = Math
				.sin(ai), aG = Math.tan(an / 4), aJ = 4 / 3 * az * aG, aH = 4
				/ 3 * ax * aG, aP = [ ap, aU ], aO = [ ap + aJ * aS,
				aU - aH * ah ], aN = [ ao + aJ * aR, aT - aH * ag ], aL = [ ao,
				aT ];
		aO[0] = 2 * aP[0] - aO[0];
		aO[1] = 2 * aP[1] - aO[1];
		if (ar) {
			...
			for (var aM = 0, aC = aD[C]; aM < aC; aM++) {
				aB[aM] = aM % 2 ? aQ(aD[aM - 1], aD[aM], e).y : aQ(aD[aM],
						aD[aM + 1], e).x;
			}
			return aB;
		}
	}, E = ab(function(i, e, at, aq, af, ae, ah, ag, am) {
		var ak = Math.pow(1 - am, 3) * i + Math.pow(1 - am, 2) * 3 * am * at
				+ (1 - am) * 3 * am * am * af + Math.pow(am, 3) * ah, ai = Math
				.pow(1 - am, 3)
				* e
				+ Math.pow(1 - am, 2)
				* 3
				...
				* 3
				* am
				* am * ae + Math.pow(am, 3) * ag, ao = i + 2 * am * (at - i)
				+ am * am * (af - 2 * at + i), an = e + 2 * am * (aq - e) + am
				* am * (ae - 2 * aq + e), ar = at + 2 * am * (af - at) + am
				* am * (ah - 2 * af + at), ap = aq + 2 * am * (ae - aq) + am
				* am * (ag - 2 * ae + aq), al = (1 - am) * i + am * at, aj = (1 - am)
				* e + am * aq, ad = (1 - am) * af + am * ah, R = (1 - am) * ae
				+ am * ag;
		return {
			x : ak,
			y : ai,
		...
		var ao = (aq - 2 * ad + i) - (am - 2 * aq + ad), al = 2 * (ad - i) - 2
				* (aq - ad), ai = i - ad, ag = (-al + Math.sqrt(al * al - 4
				* ao * ai))
				/ 2 / ao, ae = (-al - Math.sqrt(al * al - 4 * ao * ai)) / 2
				/ ao, ak = [ e, aj ], an = [ i, am ], ah = E(i, e, ad, R, aq,
				ap, am, aj, ag > 0 && ag < 1 ? ag : 0), af = E(i, e, ad, R, aq,
				ap, am, aj, ae > 0 && ae < 1 ? ae : 0);
			...
			function(ap, ak) {
				var ae = z(ap), al = ak && z(ak), am = {
					...
					case "M":
						...
						break;
					case "A":
						aq = [ "C" ].concat(u[h](0, [ ar.x, ar.y ].concat(aq
								.slice(1))));
						break;
					case "S":
					...
					case "T":
						...
						aq = [ "C" ].concat(A(ar.x, ar.y, ar.qx, ar.qy, aq[1],
								aq[2]));
						break;
					case "Q":
						ar.qx = aq[1];
						ar.qy = aq[2];
						aq = [ "C" ].concat(A(ar.x, ar.y, aq[1], aq[2], aq[3],
								aq[4]));
						break;
					case "L":
				...
				for (var ai = 0, an = Math.max(ae[C], al && al[C] || 0); ai < an; ai++) {
					...
					ad(ae, al, am, e, ai);
					ad(al, ae, e, am, ai);
					var ah = ae[ai], ao = al && al[ai], af = ah[C], aj = al
							&& ao[C];
					am.x = ah[af - 2];
					am.y = ah[af - 1];
	...
	if (f.svg) {
		...
		var Q = function(ad, an, e) {
					...
					.replace(/^r(?:\(([^,]+?)\s*,\s*([^\)]+?)\))?/,
							function(aq, i, ar) {
								...
								if (i && ar) {
									ah = P[s](i);
									af = P[s](ar);
									if (Math.pow(ah - 0.5, 2)
											+ Math.pow(af - 0.5, 2) > 0.25) {
										af = Math.sqrt(0.25 - Math.pow(
												ah - 0.5, 2)) + 0.5;
									}
								}
								return "";
			...
			if (ak == "linear") {
				...
				var ae = [ 0, 0, Math.cos(ag * Math.PI / 180),
						Math.sin(ag * Math.PI / 180) ], am = 1 / (Math.max(Math
						.abs(ae[2]), Math.abs(ae[3])) || 1);
				ae[2] *= am;
				ae[3] *= am;
				if (ae[2] < 0) {
					ae[0] = -ae[2];
			...
			for (var ai = 0, ao = aj[C]; ai < ao; ai++) {
				var al = F("stop");
				F(al, {
					offset : aj[ai].offset ? aj[ai].offset : !ai ? "0%"
							: "100%",
		...
		var J = function(al, av) {
			var ao = {
				...
				"--." : [ 8, 3, 1, 3 ],
				"--.." : [ 8, 3, 1, 3, 1, 3 ]
			}, aq = al.node, am = al.attrs, ai = al.attr("rotation"), af = function(
					aC, aB) {
			...
			for ( var ap in av) {
				if (av.hasOwnProperty(ap)) {
					...
					case "clip-rect":
						...
						if (i[C] == 4) {
							al.clip
									&& al.clip.parentNode.parentNode
											.removeChild(al.clip.parentNode);
						...
						if (!an) {
							var au = g.getElementById(aq.getAttribute(
					...
					case "translation":
						var ag = (an + "").split(ac);
						al.translate((+ag[0] + 1 || 2) - 1,
								(+ag[1] + 1 || 2) - 1);
						break;
					case "scale":
						var ag = (an + "").split(ac);
						al.scale(+ag[0] || 1, +ag[1] || +ag[0] || 1, +ag[2]
								|| null, +ag[3] || null);
						break;
					case "fill":
						var ae = (an + "")
						...
						if (!f.getRGB(an).error) {
							delete av.gradient;
							delete am.gradient;
							if (!f.is(am.opacity, "undefined")
									&& f.is(av.opacity, "undefined")) {
								aq.style.opacity = am.opacity;
								F(aq, {
									opacity : am.opacity
								});
							}
							if (!f.is(am["fill-opacity"], "undefined")
									&& f.is(av["fill-opacity"], "undefined")) {
					...
					case "fill-opacity":
						if (am.gradient) {
							var e = g.getElementById(aq.getAttribute("fill")
									.replace(/^url\(#|\)$/g, ""));
							if (e) {
		...
		var X = function(e, ae) {
			if (e.type != "text"
					|| !("text" in ae || "font" in ae || "font-size" in ae
							|| "x" in ae || "y" in ae)) {
				return;
			}
			var aj = e.attrs, R = e.node, al = R.firstChild ? P[r](
					g.defaultView.getComputedStyle(R.firstChild, "")
		...
		n[o].rotate = function(i, e, ad) {
			if (i == null) {
				if (this._.rt.cx) {
					return [ this._.rt.deg, this._.rt.cx, this._.rt.cy ]
							.join(" ");
				}
			...
			if (this._.rt.deg) {
				this.transformations[0] = f.format("rotate({0} {1} {2})",
						this._.rt.deg, e, ad);
				this.clip
						&& F(this.clip, {
							transform : f.format("rotate({0} {1} {2})",
									-this._.rt.deg, e, ad)
						});
			} else {
		...
		n[o].getBBox = function() {
			...
			if (this.type == "text") {
				...
				for (var R = 0, ae = this.node.getNumberOfChars(); R < ae; R++) {
					...
					(ag.y < ah.y) && (ah.y = ag.y);
					(ag.y + ag.height - ah.y > ah.height)
							&& (ah.height = ag.y + ag.height - ah.y);
				}
			}
			ad && this.hide();
			return ah;
	...
	if (f.vml) {
		...
		var J = function(aj, an) {
			...
			if (an.scale) {
				...
				aj.scale(+ad[0] || 1, +ad[1] || +ad[0] || 1, +ad[2] || null,
						+ad[3] || null);
			}
			if ("clip-rect" in an) {
			...
			if (aj.type == "image" && an.opacity) {
				al.filterOpacity = " progid:DXImageTransform.Microsoft.Alpha(opacity="
						+ (an.opacity * 100) + ")";
			...
			an["font-family"]
					&& (ag.fontFamily = '"'
							+ an["font-family"].split(",")[0].replace(
									/^['"]+|['"]+$/g, "") + '"');
			an["font-size"] && (ag.fontSize = an["font-size"]);
			an["font-weight"] && (ag.fontWeight = an["font-weight"]);
			an["font-style"] && (ag.fontStyle = an["font-style"]);
			if (an.opacity != null || an["stroke-width"] != null
					|| an.fill != null || an.stroke != null
					|| an["stroke-width"] != null
					|| an["stroke-opacity"] != null
					|| an["fill-opacity"] != null
					|| an["stroke-dasharray"] != null
					|| an["stroke-miterlimit"] != null
					|| an["stroke-linejoin"] != null
					|| an["stroke-linecap"] != null) {
				al = aj.shape || al;
				var am = (al.getElementsByTagName("fill") && al
						.getElementsByTagName("fill")[0]), ap = false;
				!am && (ap = am = Z("fill"));
				if ("fill-opacity" in an || "opacity" in an) {
					var i = ((+ao["fill-opacity"] + 1 || 2) - 1)
							* ((+ao.opacity + 1 || 2) - 1);
					i < 0 && (i = 0);
					i > 1 && (i = 1);
					am.opacity = i;
				...
				var ae = (al.getElementsByTagName("stroke") && al
						.getElementsByTagName("stroke")[0]), aq = false;
				!ae && (aq = ae = Z("stroke"));
				if ((an.stroke && an.stroke != "none") || an["stroke-width"]
						|| an["stroke-opacity"] != null
						|| an["stroke-dasharray"] || an["stroke-miterlimit"]
						|| an["stroke-linejoin"] || an["stroke-linecap"]) {
					ae.on = true;
				}
				(an.stroke == "none" || ae.on == null || an.stroke == 0 || an["stroke-width"] == 0)
						&& (ae.on = false);
				ae.on && an.stroke && (ae.color = f.getRGB(an.stroke).hex);
				var i = ((+ao["stroke-opacity"] + 1 || 2) - 1)
						* ((+ao.opacity + 1 || 2) - 1);
				i < 0 && (i = 0);
				i > 1 && (i = 1);
				ae.opacity = i;
				an["stroke-linejoin"]
						&& (ae.joinstyle = an["stroke-linejoin"] || "miter");
				...
				an["stroke-width"]
						&& (ae.weight = (P[s](an["stroke-width"]) || 1) * 12 / 16);
			...
			if (at.type == "text") {
				...
				at.node.string
						&& (at.paper.span.innerHTML = at.node.string.replace(
								/</g, "&#60;").replace(/&/g, "&#38;").replace(
								/\n/g, "<br>"));
		...
		var Q = function(e, ah) {
					...
					.replace(/^r(?:\(([^,]+?)\s*,\s*([^\)]+?)\))?/,
							function(am, an, i) {
								...
								if (an && i) {
									an = P[s](an);
									i = P[s](i);
									if (Math.pow(an - 0.5, 2)
											+ Math.pow(i - 0.5, 2) > 0.25) {
										i = Math.sqrt(0.25 - Math.pow(an - 0.5,
												2)) + 0.5;
									}
									ag = an + " " + i;
								}
			...
			if (ae[C]) {
				...
				for (var ad = 0, aj = ae[C]; ad < aj; ad++) {
					ae[ad].offset
							&& al.push(ae[ad].offset + " " + ae[ad].color);
				}
				if (al[C] && ak.colors) {
		...
		n[o].setBox = function(af, ag, ae) {
			var R = this.Group.style, ah = (this.shape && this.shape.style)
					|| this.node.style;
			af = af || {};
			for ( var ai in af) {
			...
			case "image":
				...
				break;
			case "text":
				this.textpath.v = [ "m", ~~(al.x + 0.5), ", ",
						~~(al.y - 2 + 0.5), "l", ~~(al.x + 0.5) + 1, ", ",
						~~(al.y - 2 + 0.5) ].join("");
			...
			if (this.type == "path" || this.type == "text") {
				...
				this.W = ap;
				this.H = ak;
				(R.width != this.paper.width + "px")
						&& (R.width = this.paper.width + "px");
				(R.height != this.paper.height + "px")
						&& (R.height = this.paper.height + "px");
				...
				if (this.type == "rect" && this.arcsize != aq
						&& (aq || this.arcsize)) {
		...
		n[o].insertAfter = function(e) {
			if (e.Group.nextSibling) {
				e.Group.parentNode
						.insertBefore(this.Group, e.Group.nextSibling);
			} else {
		...
		var G = function(i, e, ah, af) {
			...
			ae.style.cssText = "position:absolute;left:0;top:0;width:"
					+ i.width + "px;height:" + i.height + "px";
		...
		var M = function(i, ah, ag, ai, ad, e) {
			var ae = Z("group"), R = Z(e ? "roundrect" : "rect"), aj = (+e || 0)
					/ (Math.min(ai, ad));
			R.arcsize = aj;
			ae.style.cssText = "position:absolute;left:0;top:0;width:"
					+ i.width + "px;height:" + i.height + "px";
		...
		var L = function(i, ai, ah, aj) {
			...
			ae.coordsize = i.coordsize;
			ae.coordorigin = i.coordorigin;
			ak.v = f.format("m{0},{1}l{2},{1}", ~~(ai + 0.5), ~~(ah + 0.5),
					~~(ai + 0.5) + 1);
			ak.textpathok = true;
			ag.width = i.width;
			ag.height = i.height;
		...
		(function(e) {
			n[o][e] = function(i) {
				if (f.is(i, "function")) {
					...
					this.events[e][i].push(k(this.shape || this.node, e, i,
							this));
				}
				return this;
			};
			n[o]["un" + e] = function(i) {
				this.events && this.events[e] && this.events[e][i]
						&& this.events[e][i][C] && this.events[e][i].shift()()
						&& !this.events[e][i][C] && delete this.events[e][i];
			};
		})(y[V]);
	}
	...
	b.set = function(e) {
		arguments[C] > 1
				&& (e = Array[o].splice.call(arguments, 0, arguments[C]));
		return new l(e);
	};
	b.setSize = I;
	n[o].stop = function() {
	...
	n[o].scale = function(ao, an, ad, R) {
		...
		if (ao != 0) {
			var al = this.getBBox(), ai = al.x + al.width / 2, af = al.y
					+ al.height / 2, aE = ao / this._.sx, aD = an / this._.sy;
			ad = (+ad || ad == 0) ? ad : ai;
			R = (+R || R == 0) ? R : af;
			var ak = ~~(ao / Math.abs(ao)), ah = ~~(an / Math.abs(an)), aw = this.node.style, aH = ad
					+ (ai - ad) * ak * aE, aG = R + (af - R) * ah * aD;
			switch (this.type) {
			case "rect":
			case "image":
				var aj = aF.width * ak * aE, av = aF.height * ah * aD, am = aF.r
						* Math.min(aE, aD), ag = aH - aj / 2, ae = aG - av / 2;
				this.attr({
					width : aj,
					height : av,
					x : ag,
			...
			case "path":
				...
				var e = H(ay), at = aH - e.x - e.width / 2, aq = aG - e.y
						- e.height / 2;
				ay[0][1] += at;
				ay[0][2] += aq;
				this.attr({
			...
			if (this.type in {
				...
				if (this.transformations) {
					...
					this.node.setAttribute("transform", this.transformations
							.join(" "));
	...
	f.easing_formulas = {
		...
		elastic : function(R) {
			...
			var i = 0.3, e = i / 4;
			return Math.pow(2, -10 * R) * Math.sin((R - e) * (2 * Math.PI) / i)
					+ 1;
		},
		bounce : function(ad) {
	...
	n[o].animate = function(ax, an, am, ae) {
					...
					case "path":
						...
						for (var at = 0, ai = aq[ao][C]; at < ai; at++) {
							...
							for (var ar = 1, av = aq[ao][at][C]; ar < av; ar++) {
								ak[ao][at][ar] = (R[ao][at][ar] - aq[ao][at][ar])
										/ an;
							}
						}
						break;
					case "csv":
						var e = (ax[ao] + "").split(ac), ah = (aq[ao] + "")
								.split(ac);
						switch (ao) {
						case "translation":
							aq[ao] = [ 0, 0 ];
		...
		(function au() {
			...
			if (az < an) {
				var aF = f.easing_formulas[am] ? f.easing_formulas[am](az / an)
						: az / an;
				for ( var aD in aq) {
					if (aq.hasOwnProperty(aD)) {
						...
						case "colour":
							ay = "rgb("
									+ [
											aw(~~(aq[aD].r + aF * an * ak[aD].r + 0.5)),
											aw(~~(aq[aD].g + aF * an * ak[aD].g + 0.5)),
											aw(~~(aq[aD].b + aF * an * ak[aD].b + 0.5)) ]
											.join(",") + ")";
							break;
						case "path":
							ay = [];
						...
						case "csv":
							switch (aD) {
							case "translation":
								var aG = ak[aD][0] * (az - al), aE = ak[aD][1]
										* (az - al);
								aj.x += aG;
								aj.y += aE;
								ay = [ aG, aE ].join(" ");
			...
			for ( var ae in i.glyphs) {
				if (i.glyphs.hasOwnProperty(ae)) {
					...
					ad.glyphs[ae] = {
						...
						k : {},
						d : af.d && "M"
								+ af.d.replace(/[mlcxtrv]/g, function(ah) {
									return {
										l : "L",
										c : "C",
										x : "z",
	...
	b.getFont = function(ai, aj, R, ae) {
		...
		var af = f.fonts[ai];
		if (!af) {
			var ad = new RegExp("(^|\\s)" + ai.replace(/[^\w\d\s+!~.:_-]/g, "")
					+ "(\\s|$)", "i");
		...
		if (af) {
			for (var ah = 0, ak = af[C]; ah < ak; ah++) {
				ag = af[ah];
				if (ag.face["font-weight"] == aj
						&& (ag.face["font-style"] == R || !ag.face["font-style"])
						&& ag.face["font-stretch"] == ae) {
					break;
				}
			}
		}
	...
	f.format = function(i) {
		var e = f.is(arguments[1], "array") ? [ 0 ].concat(arguments[1])
				: arguments;
		i && f.is(i, "string") && e[C] - 1
				&& (i = i.replace(/\{(\d+)\}/g, function(ad, R) {
					return e[++R] == null ? "" : e[R];
				}));
		return i || "";
	};
